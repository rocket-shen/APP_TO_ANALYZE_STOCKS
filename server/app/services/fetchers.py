...
from app.utils.tools import get_stock_name
...
async def fetch_xq_holders(symbol: str) -> list[dict]:
    """
    通用雪球股东数据抓取器
    ...
    stock_code = symbol[-6:]
    stock_name = get_stock_name(stock_code) or "未知"
    ...
    async with httpx.AsyncClient(timeout=settings.TIMEOUT) as client:
        try:
            ...
            results = []
            for item in data_list:
                change_date = item.get("timestamp")
                if not change_date:
                    continue

                item["symbol"] = symbol.upper()
                item["stock_name"] = stock_name
                item["change_date"] = datetime.fromtimestamp(
                    change_date / 1000
                ).strftime("%Y-%m-%d")

                results.append(item)

            logger.info(f"成功抓取 {symbol} : {len(results)} 條記錄")
            return results

        except httpx.HTTPStatusError as e:
            logger.error(f"HTTP錯誤 {e.response.status_code}: {symbol} holders")
            raise
        except Exception as e:
